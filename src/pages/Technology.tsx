...
function CustomCursor() {
    ...
    const rx = useSpring(cx, { stiffness: 120, damping: 16 }); const ry = useSpring(cy, { stiffness: 120, damping: 16 })
    const [hover, setHover] = useState(false); const [dark, setDark] = useState(false)
    useEffect(() => {
        if (typeof window === "undefined") return
    ...
    return (
        <>
            <motion.div aria-hidden="true" style={{ position: "fixed", top: 0, left: 0, pointerEvents: "none", zIndex: 9999, x: cx, y: cy, translateX: "-50%", translateY: "-50%", width: hover ? 10 : 6, height: hover ? 10 : 6, borderRadius: "50%", background: dark ? T.orange : T.maroon, transition: "width 0.2s, height 0.2s, background 0.3s" }} />
            <motion.div aria-hidden="true" style={{ position: "fixed", top: 0, left: 0, pointerEvents: "none", zIndex: 9998, x: rx, y: ry, translateX: "-50%", translateY: "-50%", width: hover ? 52 : 32, height: hover ? 52 : 32, borderRadius: "50%", border: `1.5px solid ${dark ? "rgba(229,86,2,0.5)" : "rgba(69,14,20,0.3)"}`, background: hover ? (dark ? "rgba(229,86,2,0.08)" : "rgba(69,14,20,0.06)") : "transparent", transition: "width 0.3s, height 0.3s, border-color 0.3s" }} />
        </>
    )
}
